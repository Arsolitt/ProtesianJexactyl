...
export default ({ backup, className }: Props) => {
    ...
    return (
        <GreyRowBox css={tw`flex-wrap md: flex-nowrap items-center`} className={className}>
            <div
                css={tw`flex items-center truncate w-full
                    md: flex-1`}
            >
                <div css={tw`mr-4`}>
                ...
                <div css={tw`flex flex-col truncate`}>
                    <div css={tw`flex items-center text-sm mb-1`}>
                        ...
                        {backup.completedAt !== null && backup.isSuccessful && (
                            <span
                                css={tw`ml-3 text-neutral-300 text-xs font-extralight hidden
                                    sm: inline`}
                            >
            ...
            <div
                css={tw`flex-1 md: flex-none
                    md: w-48 mt-4
                    md: mt-0
                    md: ml-8
                    md: text-center`}
            >
                <p title={format(backup.createdAt, 'ddd, MMMM do, yyyy HH:mm:ss', { locale: ru })} css={tw`text-sm`}>
                    {formatDistanceToNow(backup.createdAt, { includeSeconds: true, addSuffix: true, locale: ru })}
                </p>
                <p css={tw`text-2xs text-neutral-500 uppercase mt-1`}>Создан</p>
            </div>
