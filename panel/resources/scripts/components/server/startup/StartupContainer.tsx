    ...
    return !data ? (
        ...
        >
            <div className={'md:flex j-up'}>
                ...
                <TitledGreyBox title={'ПО для запуска'} css={tw`flex-1 lg:flex-none lg:w-1/3 mt-8 md:mt-0 md:ml-10`}>
                    {Object.keys(data.dockerImages).length > 1 && !isCustomImage ? (
                        <>
                            ...
                            <p css={tw`text-xs text-neutral-300 mt-2`}>
                                Можно выбрать образ, из которого будет запускаться твой сервер
                            </p>
                        </>
                    ) : (
                        <>
                            ...
                            {isCustomImage && (
                                <p css={tw`text-xs text-neutral-300 mt-2`}>
                                    Образ был вручную задан администратором и ты его не можешь изменить руками. Если это
                                    ошибка - обратись в поддержку.
                                </p>
                            )}
                        </>
                    )}
